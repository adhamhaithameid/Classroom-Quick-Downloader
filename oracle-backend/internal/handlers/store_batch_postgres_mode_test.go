...
func TestIngestBatchHandlerV4_PostgresPrimaryMirrorFailureStillReturnsSuccess(t *testing.T) {
	dsn := os.Getenv("POSTGRES_DSN")
	if dsn == "" {
		t.Skip("POSTGRES_DSN not set")
	}
	...
	if _, err := sqlDB.Exec(`DROP TABLE batches`); err != nil {
		t.Fatalf("failed to induce sqlite mirror failure: %v", err)
	}

	batchID := "batch-postgres-primary-mirror-failure"
	if _, err := postgresDB.Exec(`DELETE FROM pg_ingest_batches WHERE batch_id = $1`, batchID); err != nil {
		t.Fatalf("failed to cleanup pg_ingest_batches: %v", err)
	}
	if _, err := postgresDB.Exec(`DELETE FROM raw_ingest_events WHERE idempotency_key = $1`, "batch:"+batchID); err != nil {
	...
	IngestBatchHandlerV4(sqlDB, postgresDB, "secret").ServeHTTP(rr, req)
	if rr.Code != http.StatusOK {
		t.Fatalf("expected 200 on sqlite mirror failure, got %d: %s", rr.Code, rr.Body.String())
	}
	...
	if pgCount != 1 {
		t.Fatalf("expected postgres ingest batch row despite sqlite mirror failure, got %d", pgCount)
	}

	var mirrorFailures int64
	if err := sqlDB.QueryRow(`SELECT COUNT(*) FROM pipeline_failure_logs WHERE stage = 'ingest_mirror' AND batch_id = ?`, batchID).Scan(&mirrorFailures); err != nil {
		t.Fatalf("query pipeline_failure_logs failed: %v", err)
	}
	if mirrorFailures == 0 {
