...
describe('springConverged', () => {
  it('is true only when position and velocity hug the target', () => {
    expect(springConverged({ x: 4, y: -2, vx: 0, vy: 0 }, { x: 4, y: -2 })).toBe(true);
    expect(springConverged({ x: 4.5, y: -2, vx: 0, vy: 0 }, { x: 4, y: -2 })).toBe(false);
    expect(springConverged({ x: 4, y: -2, vx: 1, vy: 0 }, { x: 4, y: -2 })).toBe(false);
  });

  it('ignores engagement: a parked hover converges at its target too', () => {
    // The action uses this to stop the rAF loop while the cursor sits still
    // inside the radius; there is no engaged flag to wake it.
    expect(springConverged({ x: 3, y: 0, vx: 0, vy: 0 }, { x: 3, y: 0 })).toBe(true);
  });
});

describe('locked-in hover constants', () => {
  it('lift and scale match the approved prototype feel', () => {
    expect(HOVER_LIFT_PX).toBe(-2);
    expect(HOVER_SCALE).toBe(1.02);
  });
});
