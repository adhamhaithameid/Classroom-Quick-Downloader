...
export class DebugPanel {
  ...
  refresh(): void {
    ...
    const createRow = (key: string, value: string, valueClass = ''): HTMLDivElement => {
      ...
      valueSpan.className = `cqd-dbg-val ${valueClass}`.trim();
    ...
    engineSection.appendChild(
      createRow('Mode', mode || 'none', mode === 'shadow' ? 'warn' : mode === 'v2' ? 'good' : ''),
    );
    engineSection.appendChild(createRow('View', currentView || 'none'));
    const activeEnginesLabel = activeEngines.map((e) => `${e.name} v${e.version}`).join(', ') || 'none';
    engineSection.appendChild(createRow('Active Engines', activeEnginesLabel));
    ...
    if (mode === 'shadow' && shadowReport) {
      ...
      shadowSection.appendChild(
        createRow('Match Rate', `${shadowReport.matchPercentage.toFixed(1)}%`, matchClass),
      );
      shadowSection.appendChild(createRow('Posts Analyzed', String(shadowReport.postsAnalyzed)));
      shadowSection.appendChild(
        createRow(
          'Mismatches',
          String(shadowReport.mismatchCount),
          shadowReport.mismatchCount > 0 ? 'bad' : 'good',
        ),
      );
    ...
    if (v2Engine) {
      ...
      const flagsSection = createSection(
        `V2 Flags (${flagDecisions.length} decisions, ${trackedPosts.length} posts)`,
      );
      ...
      if (flagDecisions.length === 0) {
        ...
        for (const decision of shown) {
          ...
          flag.className = `cqd-dbg-flag ${verdictClass}`;

          flag.appendChild(
            createRow(`${decision.postId.slice(0, 12)}…`, String(decision.finalVerdict)),
          );
          ...
          scores.textContent = `C:${decision.commentScore} E:${decision.editedScore}`;
          const confidence = document.createElement('span');
          confidence.textContent = `conf: ${decision.confidence}`;

          detailsRow.appendChild(scores);
          detailsRow.appendChild(confidence);
          flag.appendChild(detailsRow);

          flagsSection.appendChild(flag);
        }
        if (flagDecisions.length > 10) {
          const more = document.createElement('div');
          more.style.color = '#666';
          more.style.textAlign = 'center';
          more.textContent = `+${flagDecisions.length - 10} more`;
          flagsSection.appendChild(more);
        }
      }
      fragment.appendChild(flagsSection);
    }
    ...
    if (v2Engine) {
      ...
      const modelSection = createSection('V2 Model');
      modelSection.appendChild(createRow('Tracked Posts', String(posts.length)));
      modelSection.appendChild(
        createRow(
          'Total Files',
          String(posts.reduce((sum, post) => sum + post.files.length, 0)),
        ),
      );
      modelSection.appendChild(createRow('Placements', String(placements.length)));
